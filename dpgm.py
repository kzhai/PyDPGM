...
class VariationalInference(object):
    ...
    def _initialize(self, X):
        ...
        sq_dist = (numpy.sum(X ** 2, axis=1)[:, None]
                   - 2.0 * (X @ seed_means.T)
    ...
    def _expectations(self):
        ...
        E_logdet_Lambda = (scipy.special.digamma((self._nu[:, None] + 1 - i[None, :]) / 2.0).sum(axis=1)
    ...
    def _elbo(self, Nk, xbar, S, resp, log_resp,
        ...
        ln_beta_fn = (scipy.special.gammaln(self._gamma1) + scipy.special.gammaln(self._gamma2)
...
def fit_dpgm_vi(data, alpha=1.0, truncation=50, max_iter=200, tol=1e-4,
    ...
    model = VariationalInference(truncation=truncation, alpha=alpha, max_iter=max_iter,
